# using script available at: https://www.thepythoncode.com/article/make-process-monitor-python#:~:text=as%20an%20administrator).-,process.,called%20it%20will%20return%200.0.
...
def construct_dataframe(processes, columns, sort_by, descending):
...
def extract_suspicious(summary = False):
    import argparse
    ...
    df = construct_dataframe(processes, columns, sort_by, descending)
    if summary:
        if n == 0:
            print(df.to_string())
        elif n > 0:
